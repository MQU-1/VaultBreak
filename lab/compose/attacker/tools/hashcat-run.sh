...
# bundled sample wordlist. Just a few seconds for these two hashes.
#
...
# strip usernames -> bare hash list
awk -F: '{print $2}' "$HASHES" > /tmp/hashes-only.txt
...
echo "[*] recovered:"
echo "======================================================================"
for u in $(awk -F: '{print $1}' "$HASHES"); do
